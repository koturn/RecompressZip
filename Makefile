...
deploy-$(TARGET_NET10):
	-dotnet publish -c $(BUILD_CONFIG) -f $(TARGET_NET10) -r win-x64 --no-self-contained \
		...
		-p:PublishAot=false \
		$(MAIN_PROJECT_FILE)
	xcopy /d /S /Y "$(SOLUTION_NAME)\bin\$(BUILD_CONFIG)\$(TARGET_NET10)\x64\*.dll" "$(ARTIFACTS_BASEDIR)\$(ARTIFACTS_SUBDIR_BASENAME)-$(TARGET_NET10)\x64\"
	xcopy /d /S /Y "$(SOLUTION_NAME)\bin\$(BUILD_CONFIG)\$(TARGET_NET10)\x86\*.dll" "$(ARTIFACTS_BASEDIR)\$(ARTIFACTS_SUBDIR_BASENAME)-$(TARGET_NET10)\x86\"
...
deploy-$(TARGET_NET10)$(SINGLE_SUFFIX):
	-dotnet publish -c $(BUILD_CONFIG) -f $(TARGET_NET10) -r win-x64 --self-contained \
		...
		$(MAIN_PROJECT_FILE)
	xcopy /d /S /Y "$(SOLUTION_NAME)\bin\$(BUILD_CONFIG)\$(TARGET_NET10)\x64\*.dll" "$(ARTIFACTS_BASEDIR)\$(ARTIFACTS_SUBDIR_BASENAME)-$(TARGET_NET10)$(SINGLE_SUFFIX)\x64\"
	xcopy /d /S /Y "$(SOLUTION_NAME)\bin\$(BUILD_CONFIG)\$(TARGET_NET10)\x86\*.dll" "$(ARTIFACTS_BASEDIR)\$(ARTIFACTS_SUBDIR_BASENAME)-$(TARGET_NET10)$(SINGLE_SUFFIX)\x86\"
